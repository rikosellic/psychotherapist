...
可向 Agent 提供心理咨询记录、初访访谈、督导材料或个案报告草稿。输出统一分为三种形态：

1. **自由分析（默认）**：适用于“帮我分析”“你怎么看”“给点思路”“下一步怎么做”等请求。它不套报告模板，而是按照当前启用的 `SOUL` 语气，像治疗师同行之间共同看材料一样进行有来有回的分析和讨论。材料再完整，也不会因此自动输出正式报告。
2. **结构化报告**：仅在用户明确要求可归档、提交或汇报的正式文稿时启用。Agent 会根据 CBT、心理动力学、家庭系统或整合取向自动选择模板；“简要一些”“突出督导难点”“使用某一流派”等只作为报告侧重点，不再构成独立模式。
3. **干预指导**：适用于用户明确要求系统、完整的后续干预方案。统一使用干预指导模板；如果用户已提供概念化则不重复摘要，如果尚未提供则自动附一段必要的简要概念化。
